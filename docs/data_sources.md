...
The repository is public, but the maintainer's private legacy bootstrap is not a
public source. Publicly reproducible source work should focus on APIs, open
metadata, and reviewed snapshots generated by the project.
...
- the first MVP PubMed discovery command stages date-windowed candidates and can
  be run month-by-month from January 2024 through the current date in the
...
- How many candidate studies exist beyond the reconciled private bootstrap or
