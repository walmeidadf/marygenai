...
def parse_doi(article: etree._Element) -> str | None:
    for article_id in article.xpath(".//ArticleIdList/ArticleId[@IdType='doi']"):
...
def output_paths(raw_dir: Path, normalized_dir: Path, query_slug: str) -> dict[str, Path]:
...
def print_summary(paths: dict[str, Path], summary: dict[str, Any]) -> None:
    table = Table(title="PubMed validation summary")
...
def run(
    ...
    try:
        ...
        delay_seconds = 0.11 if pubmed_api_key else 0.34
        time.sleep(delay_seconds)
        xml_text = client.fetch_xml(pmids) if pmids else "<PubmedArticleSet />"
    finally:
        client.close()

    paths = output_paths(resolved_raw_dir, resolved_normalized_dir, slugify_query(query))
